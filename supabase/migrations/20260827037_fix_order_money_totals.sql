-- =============================================================================
-- 20260827037_fix_order_money_totals.sql
--
-- Fix: create_order_from_checkout_rpc calculated order totals from cart_lines
-- (which stores unit_price_snapshot in kobo / minor units) and then mixed
-- those values with checkout_sessions.*_total fields (which are in Naira /
-- major units, stored as INTEGER).  This produced wildly incorrect order
-- totals (e.g. a ₦1,700 order being recorded as ₦21,500 in the admin).
--
-- Root cause (unit model, proven from schema + service code):
--   cart_lines.unit_price_snapshot  → money_amount (bigint) → KOBO
--   cart.subtotal (enrichCart())    → (price / 100 * qty)   → NAIRA
--   checkout_sessions.*_total       → INTEGER                → NAIRA
--   orders.*_total                  → money_amount (bigint)  → KOBO
--   order_lines.unit_price_snapshot → money_amount (bigint)  → KOBO
--   order_lines.line_total          → money_amount (bigint)  → KOBO
--
-- Fix: source all five monetary totals exclusively from checkout_sessions
-- (the authoritative, payment-time-locked source of truth) and convert each
-- from Naira → Kobo (× 100) before writing to orders.*_total columns.
--
-- order_lines.unit_price_snapshot and line_total are unaffected — they come
-- directly from cart_lines.unit_price_snapshot which is already in Kobo.
-- =============================================================================
...
DECLARE
    ...
    v_order_id              UUID;
    v_order_number          TEXT;
    -- Monetary totals sourced from checkout_sessions (Naira) then converted to Kobo
...
BEGIN
    ...
    -- 2. Fetch cart (needed to walk cart_lines for order_lines insertion)
    SELECT * INTO v_cart
    FROM carts
    ...
    -- 3. Guard: cart must be non-empty (line count check, not a monetary recalculation)
    IF NOT EXISTS (SELECT 1 FROM cart_lines WHERE cart_id = v_cart.id) THEN
        RAISE EXCEPTION 'Cart % is empty', v_cart.id;
    END IF;

    -- 4. Source all monetary totals exclusively from the locked checkout_session.
    --    checkout_sessions.*_total columns are INTEGER in Naira (major units).
    --    orders.*_total columns are money_amount (bigint) in Kobo (minor units).
    --    Conversion: multiply by 100 (1 Naira = 100 Kobo).
    ...
    -- Sanity guard: grand_total must be positive (should always hold for a paid session)
    ...
    IF v_session.fulfilment_method_id IS NOT NULL THEN
        ...
        -- 5b. Extract destination state and city for zone matching
        ...
        -- 5c. Match shipping zone using Phase B deterministic specificity:
        --     Direct region match takes precedence (zone with fewer regions = most specific)
        ...
        -- 5d. Match shipping rate for method and zone
        ...
        ELSE
            -- Handle Store Pickup methods (₦0 cost, no explicit rate row in shipping_rates)
    ...
    -- 7. Create Order
    --    All *_kobo variables are in Kobo, matching orders money_amount (bigint) domain.
    INSERT INTO orders (
        order_number,
        customer_id,
        guest_contact,
    ...
    -- 8. Insert Order Lines & deduct stock
    --    unit_price_snapshot and line_total are written in Kobo — they come directly
    --    from cart_lines.unit_price_snapshot which is already in minor units (money_amount).
    FOR v_line IN (
