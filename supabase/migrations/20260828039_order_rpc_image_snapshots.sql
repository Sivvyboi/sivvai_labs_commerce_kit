-- =============================================================================
-- 20260828039_order_rpc_image_snapshots.sql
--
-- Fix: create_order_from_checkout_rpc previously inserted NULL for
-- order_lines.image_url_snapshot during checkout order creation.
--
-- This migration updates create_order_from_checkout_rpc to query each product's
-- primary image from product_images (ordered by is_primary DESC, display_order ASC)
-- and snapshot the image URL into order_lines.image_url_snapshot.
--
-- Also backfills any existing order_lines where image_url_snapshot IS NULL.
-- =============================================================================
...
BEGIN
    ...
    -- 8. Insert Order Lines with primary product image snapshot & deduct stock
    FOR v_line IN (
...
-- Backfill any existing order lines that may have null image snapshots
UPDATE order_lines ol
SET image_url_snapshot = sub.url
FROM (
    SELECT DISTINCT ON (ol2.id) ol2.id, pi.url
    FROM order_lines ol2
    JOIN product_variants pv ON pv.id = ol2.variant_id
    JOIN product_images pi ON pi.product_id = pv.product_id
    ORDER BY ol2.id, pi.is_primary DESC, pi.display_order ASC, pi.created_at ASC
) sub
WHERE ol.id = sub.id AND ol.image_url_snapshot IS NULL;
