-- =============================================================================
-- 20260910066_atomic_payment_attempt_order_linkage.sql
-- Atomic Payment Attempt Linkage & Idempotent Checkout Completion
--
-- 1. Atomically links payment_attempts.order_id, marks status = 'confirmed',
--    and sets confirmed_at = NOW() inside the create_order_from_checkout_rpc
--    transaction BEFORE it commits.
-- 2. Uses structured error codes (CHECKOUT_ALREADY_COMPLETED, PAYMENT_ALREADY_CONFIRMED)
--    so callers can cleanly recover already-created orders without racing against
--    out-of-transaction Node.js updates.
-- =============================================================================
...
BEGIN
    ...
    IF v_payment_attempt.status = 'confirmed' THEN
        RAISE EXCEPTION 'PAYMENT_ALREADY_CONFIRMED: Payment attempt % has already been confirmed',
    ...
    IF v_session.status = 'completed' THEN
        RAISE EXCEPTION 'CHECKOUT_ALREADY_COMPLETED: Checkout session % has already been completed', p_checkout_session_id;
    END IF;
    ...
    -- 9b. Atomically link payment attempt to the created order and mark confirmed
