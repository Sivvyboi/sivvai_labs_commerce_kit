 ...
 *
 * Client Component for managing categories (Create, Edit, Archive, Restore, Image Upload).
 */
...
function CategoryImageUploadModal({ category, onClose, onSuccess }: ImageUploadModalProps) {
  ...
  async function handleUpload() {
    ...
    try {
      ...
      // 3. Save the public URL to the category row
    ...
    >
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-xs text-[var(--kit-text-muted)] mt-0.5 truncate max-w-[200px]">
            {category.name}
          </p>
        </div>
        <button
...
export function CategoryManager({ initialCategories }: CategoryManagerProps) {
  ...
  // Local optimistic copy so the table updates after image changes without a full reload
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (editingCategory) {
      ...
      if (res?.success && res.category) {
        setCategories((prev) =>
          prev.map((c) => (c.id === (res.category as CategoryRow).id ? (res.category as CategoryRow) : c))
        );
        setModalOpen(false);
      }
    } else {
      ...
      if (res?.success && res.category) {
        setCategories((prev) => [...prev, res.category as CategoryRow]);
        setModalOpen(false);
      }
    }
  }
  ...
  function handleImageSuccess(updated: CategoryRow) {
    ...
    // Keep modal open but reflect changes in imageModalCategory so the preview updates
      ...
      <div className="overflow-x-auto rounded-[var(--kit-radius-lg)] border border-[var(--kit-border)] bg-[var(--kit-card)] shadow-[var(--kit-shadow-sm)]">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-[var(--kit-border)]">
            {categories.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-4 py-8 text-center text-xs text-[var(--kit-text-muted)]">
                ...
                return (
                  <tr key={cat.id} className="hover:bg-[var(--kit-surface)] transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <button
                        type="button"
                        onClick={() => setImageModalCategory(cat)}
